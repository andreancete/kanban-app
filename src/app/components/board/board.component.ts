...
export class BoardComponent implements OnInit {
  ...
  loadBoard(): void {
    this.kanbanService.getBoard(this.boardId).subscribe({
      next: (board) => {
        this.board = board;
        // Sort columns and cards by order - CREATE COPIES to avoid readonly arrays
        if (this.board.columns && Array.isArray(this.board.columns)) {
          ...
          this.board.columns.forEach((column) => {
            if (column.cards && Array.isArray(column.cards)) {
              // Create a copy of cards array for each column
  ...
  createColumn(): void {
    ...
    this.kanbanService
      ...
      .subscribe({
        next: () => {
          this.loadBoard(); // Reload to get updated data
  ...
  drop(event: CdkDragDrop<Card[]>, targetColumn: Column): void {
    // Ensure cards array exists and create mutable copies
    if (!targetColumn.cards) {
      targetColumn.cards = [];
    }

    // Ensure previous container has cards array
    ...
    if (event.previousContainer === event.container) {
      ...
      // Moving between columns - create mutable copies
      ...
      // Update both columns with the new arrays
      // Find the source column and update its cards
  ...
  focusCardDescription(): void {
    // Focus on description textarea after title input
    setTimeout(() => {
