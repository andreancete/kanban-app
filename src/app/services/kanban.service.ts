      ...
      .valueChanges.pipe(
        map((result) => {
          try {
            ...
            // Ensure all boards have consistent structure with mutable arrays
            return boards.map((board) => ({
              ...board,
              columns: [...(board.columns || [])], // Create mutable copy
            }));
          } catch (error) {
      ...
      .valueChanges.pipe(
        map((result) => {
          try {
            ...
            // Ensure board has consistent structure with mutable arrays
            return {
              ...board,
              columns: (board.columns || []).map((column) => ({
                ...column,
                cards: [...(column.cards || [])], // Create mutable copy of cards
              })),
            };
          } catch (error) {
